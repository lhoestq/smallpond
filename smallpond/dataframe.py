...
class Session(SessionBase):
    ...
    def read_parquet(
        ...
        union_by_name : bool, optional
            Whether to union columns by name when reading multiple files, by default False
        s3_region : Optional[str], optional
            AWS region for S3 access, by default None
        s3_access_key_id : Optional[str], optional
            AWS access key ID for S3 access, by default None
        s3_secret_access_key : Optional[str], optional
            AWS secret access key for S3 access, by default None
        s3_session_token : Optional[str], optional
            AWS session token for S3 access, by default None
        s3_endpoint : Optional[str], optional
        ...
        -------
        DataFrame
            A DataFrame representing the Parquet data
            
        Notes
        -----
        When reading from S3, you need to provide appropriate credentials either through:
        1. AWS environment variables (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, etc.)
        2. AWS credentials file (~/.aws/credentials)
        3. IAM role (if running on AWS infrastructure)
        4. Or explicitly via the s3_* parameters
        """
        # Check if any path is an S3 URL
        is_s3 = False
        if isinstance(paths, str):
            is_s3 = paths.startswith('s3://')
        else:
            is_s3 = any(path.startswith('s3://') for path in paths)
        
        # If reading from S3, we need to configure S3 credentials using DuckDB Secrets Manager
        if is_s3:
            import duckdb
            
            # Create the temporary secret for S3 access that will persist for the session
            secret_params = []
