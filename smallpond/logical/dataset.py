                    ...
                    try:
                        ...
                        if is_s3_path(path):
                            ...
                            try:
                                ...
                                result = duckdb.sql(f"SELECT count(*) FROM parquet_metadata('{path}')").fetchone()
                                num_rows = result[0]
                                # We can't get the exact size from S3 easily, so estimate based on rows
                                estimated_size = num_rows * 1024  # Rough estimate: 1KB per row
                                return RowRange(
                                    path=path,
                                    begin=0,
                                    end=num_rows,
                        ...
                        logger.opt(exception=ex).warning(
                            f"failed to read parquet metadata for {path}, assuming empty"
                        )
                        return RowRange(path, 0, 0, 0, 0)
